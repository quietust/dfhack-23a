...
DFhackCExport command_result plugin_init ( color_ostream &out, std::vector <PluginCommand> &commands)
{
    commands.push_back(PluginCommand(
        ...
        "  digFlood CLEAR\n"
        "    remove all stones from monitoring\n"
        "  digFlood digAll1\n"
...
void maybeExplore(color_ostream& out, MapExtras::MapCache& cache, df::coord pt, set<df::coord>& jobLocations) {
    ...
    if ( mat == -1 )
        return;
    if ( !digAll ) {
        df::matgloss_stone* stone = df::global::world->raws.matgloss.stone[mat];
        if ( autodigMaterials.find(stone->id) == autodigMaterials.end() ) {
            return;
        }
    }
...
command_result digFlood (color_ostream &out, std::vector <std::string> & parameters)
{
    ...
    for ( size_t a = 0; a < parameters.size(); a++ ) {
        ...
        for ( size_t b = 0; b < df::global::world->raws.matgloss.stone.size(); b++ ) {
            df::matgloss_stone* stone = df::global::world->raws.matgloss.stone[b];
            if ( parameters[a] == stone->id ) {
                if ( adding )
